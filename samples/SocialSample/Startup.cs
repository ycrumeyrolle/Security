        ...
        public void Configure(IApplicationBuilder app)
        {
            ...
            app.Map("/home/error", errorApp =>
            {
                errorApp.Run(async context =>
                {
                    ...
                    response.ContentType = "text/html";
                    await response.WriteAsync("<html><body>");
                    await response.WriteAsync($"A remote failure has occurred: {context.Request.Query["error"]}<br>");
                    await response.WriteAsync($"{context.Request.Query["error_description"]}<br>");
                    await response.WriteAsync("<a href=\"/\">Home</a>");
                    await response.WriteAsync("</body></html>");
                });
            });
